    ...
    try {
        ...
        throw std::out_of_range(
            fmt::format(
                ...
                arrow_dtype,
                common::arrow::to_arrow_readable(arrow_dtype)));
    }
}
...
std::pair<Attribute, std::optional<Enumeration>> ArrowAdapter::tiledb_attribute_from_arrow_schema(
    ...
    std::string_view type_metadata,
    PlatformConfig platform_config) {
    auto type = common::arrow::to_tiledb_format(arrow_schema->format, type_metadata);
    ...
    if (arrow_schema->dictionary != nullptr) {
        ...
        auto enmr_type = common::arrow::to_tiledb_format(enmr_format);
