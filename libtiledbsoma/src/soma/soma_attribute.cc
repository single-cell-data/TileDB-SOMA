...
void SOMAAttribute::_set_dim_points(
    ...
    throw TileDBSOMAError(std::format(
        "[SOMAAttribute][_set_dim_points] Column with name {} is not an index column", name()));
}
...
void SOMAAttribute::_set_dim_ranges(
    ...
    const std::any&) const {
    throw TileDBSOMAError(std::format(
        "[SOMAAttribute][_set_dim_ranges] Column with name {} is not an index column", name()));
}

void SOMAAttribute::_set_current_domain_slot(
    NDRectangle&, const std::vector<const void*>&) const {
    throw TileDBSOMAError(std::format(
        "[SOMAAttribute][_set_current_domain_slot] Column with name {} is not an index column", name()));
}

std::any SOMAAttribute::_core_domain_slot() const {
    throw TileDBSOMAError(std::format(
        "[SOMAAttribute][_core_domain_slot] Column with name {} is not an index column", name()));
}

std::any SOMAAttribute::_non_empty_domain_slot(Array&) const {
    throw TileDBSOMAError(std::format(
        "[SOMAAttribute][_non_empty_domain_slot] Column with name {} is not an index column", name()));
}

std::any SOMAAttribute::_core_current_domain_slot(
    const SOMAContext&, Array&) const {
    throw TileDBSOMAError(std::format(
        "[SOMAAttribute][_core_current_domain_slot] Column with name {} is not an index column", name()));
}

ArrowArray* SOMAAttribute::arrow_domain_slot(
    const SOMAContext&, Array&, enum Domainish) const {
    throw TileDBSOMAError(std::format(
        "[SOMAAttribute][arrow_domain_slot] Column with name {} is not an index column", name()));
}
