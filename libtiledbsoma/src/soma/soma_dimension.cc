...
void SOMADimension::_set_dim_points(ManagedQuery& query, const std::any& points) const {
    switch (dimension.type()) {
        case TILEDB_UINT8:
            ...
            break;
        case TILEDB_UINT16:
            query.select_points(dimension.name(), std::any_cast<std::span<const uint16_t>>(points));
            break;
        case TILEDB_UINT32:
            query.select_points(dimension.name(), std::any_cast<std::span<const uint32_t>>(points));
            break;
        case TILEDB_UINT64:
            query.select_points(dimension.name(), std::any_cast<std::span<const uint64_t>>(points));
            break;
        case TILEDB_INT8:
            query.select_points(dimension.name(), std::any_cast<std::span<const int8_t>>(points));
            break;
        case TILEDB_INT16:
            query.select_points(dimension.name(), std::any_cast<std::span<const int16_t>>(points));
            break;
        case TILEDB_INT32:
            query.select_points(dimension.name(), std::any_cast<std::span<const int32_t>>(points));
            break;
        ...
        case TILEDB_TIME_AS:
        case TILEDB_INT64:
            query.select_points(dimension.name(), std::any_cast<std::span<const int64_t>>(points));
            break;
        case TILEDB_FLOAT32:
            query.select_points(dimension.name(), std::any_cast<std::span<const float_t>>(points));
            break;
        case TILEDB_FLOAT64:
            query.select_points(dimension.name(), std::any_cast<std::span<const double_t>>(points));
            break;
        case TILEDB_STRING_UTF8:
        case TILEDB_STRING_ASCII:
            query.select_points(dimension.name(), std::any_cast<std::span<const std::string>>(points));
            break;
        default:
            throw TileDBSOMAError(
...
std::pair<bool, std::string> SOMADimension::_can_set_current_domain_slot(
    ...
    switch (dimension.type()) {
        case TILEDB_UINT8:
            return comparator(std::any_cast<std::array<uint8_t, 2>>(new_domain[0]));
        case TILEDB_UINT16:
            return comparator(std::any_cast<std::array<uint16_t, 2>>(new_domain[0]));
        case TILEDB_UINT32:
            return comparator(std::any_cast<std::array<uint32_t, 2>>(new_domain[0]));
        case TILEDB_UINT64:
            return comparator(std::any_cast<std::array<uint64_t, 2>>(new_domain[0]));
        case TILEDB_INT8:
            return comparator(std::any_cast<std::array<int8_t, 2>>(new_domain[0]));
        case TILEDB_INT16:
            return comparator(std::any_cast<std::array<int16_t, 2>>(new_domain[0]));
        case TILEDB_INT32:
            return comparator(std::any_cast<std::array<int32_t, 2>>(new_domain[0]));
        case TILEDB_DATETIME_YEAR:
        case TILEDB_DATETIME_MONTH:
        case TILEDB_DATETIME_WEEK:
        case TILEDB_DATETIME_DAY:
        ...
        case TILEDB_TIME_AS:
        case TILEDB_INT64:
            return comparator(std::any_cast<std::array<int64_t, 2>>(new_domain[0]));
        case TILEDB_FLOAT32:
            return comparator(std::any_cast<std::array<float_t, 2>>(new_domain[0]));
        case TILEDB_FLOAT64:
            return comparator(std::any_cast<std::array<double_t, 2>>(new_domain[0]));
        case TILEDB_STRING_ASCII:
        case TILEDB_STRING_UTF8: {
...
std::any SOMADimension::_core_domain_slot() const {
    switch (dimension.type()) {
        case TILEDB_UINT8:
            return std::make_any<std::pair<uint8_t, uint8_t>>(dimension.domain<uint8_t>());
        case TILEDB_UINT16:
            return std::make_any<std::pair<uint16_t, uint16_t>>(dimension.domain<uint16_t>());
        case TILEDB_UINT32:
            return std::make_any<std::pair<uint32_t, uint32_t>>(dimension.domain<uint32_t>());
        case TILEDB_UINT64:
            return std::make_any<std::pair<uint64_t, uint64_t>>(dimension.domain<uint64_t>());
        case TILEDB_INT8:
            return std::make_any<std::pair<int8_t, int8_t>>(dimension.domain<int8_t>());
        case TILEDB_INT16:
            return std::make_any<std::pair<int16_t, int16_t>>(dimension.domain<int16_t>());
        case TILEDB_INT32:
            return std::make_any<std::pair<int32_t, int32_t>>(dimension.domain<int32_t>());
        case TILEDB_DATETIME_YEAR:
        case TILEDB_DATETIME_MONTH:
        case TILEDB_DATETIME_WEEK:
        case TILEDB_DATETIME_DAY:
        ...
        case TILEDB_TIME_AS:
        case TILEDB_INT64:
            return std::make_any<std::pair<int64_t, int64_t>>(dimension.domain<int64_t>());
        case TILEDB_FLOAT32:
            return std::make_any<std::pair<float_t, float_t>>(dimension.domain<float_t>());
        case TILEDB_FLOAT64:
...
std::any SOMADimension::_non_empty_domain_slot(Array& array) const {
    switch (dimension.type()) {
        ...
        case TILEDB_UINT16:
            return std::make_any<std::pair<uint16_t, uint16_t>>(array.non_empty_domain<uint16_t>(dimension.name()));
        case TILEDB_UINT32:
            return std::make_any<std::pair<uint32_t, uint32_t>>(array.non_empty_domain<uint32_t>(dimension.name()));
        case TILEDB_UINT64:
            return std::make_any<std::pair<uint64_t, uint64_t>>(array.non_empty_domain<uint64_t>(dimension.name()));
        case TILEDB_INT8:
            return std::make_any<std::pair<int8_t, int8_t>>(array.non_empty_domain<int8_t>(dimension.name()));
        case TILEDB_INT16:
            return std::make_any<std::pair<int16_t, int16_t>>(array.non_empty_domain<int16_t>(dimension.name()));
        case TILEDB_INT32:
            return std::make_any<std::pair<int32_t, int32_t>>(array.non_empty_domain<int32_t>(dimension.name()));
        case TILEDB_DATETIME_YEAR:
        case TILEDB_DATETIME_MONTH:
        case TILEDB_DATETIME_WEEK:
        case TILEDB_DATETIME_DAY:
        ...
        case TILEDB_TIME_AS:
        case TILEDB_INT64:
            return std::make_any<std::pair<int64_t, int64_t>>(array.non_empty_domain<int64_t>(dimension.name()));
        case TILEDB_FLOAT32:
            return std::make_any<std::pair<float_t, float_t>>(array.non_empty_domain<float_t>(dimension.name()));
        case TILEDB_FLOAT64:
            return std::make_any<std::pair<double_t, double_t>>(array.non_empty_domain<double_t>(dimension.name()));
        case TILEDB_STRING_ASCII:
        case TILEDB_STRING_UTF8:
...
std::any SOMADimension::_non_empty_domain_slot_opt(const SOMAContext& ctx, Array& array) const {
    ...
    if (dimension.type() == TILEDB_STRING_ASCII || dimension.type() == TILEDB_STRING_UTF8) {
        ...
        ctx.tiledb_ctx()->handle_error(tiledb_array_get_non_empty_domain_var_size_from_name(
        ...
        if (is_empty) {
            return std::make_any<std::optional<std::pair<std::string, std::string>>>(std::nullopt);
        }

        var_start = malloc(size_start);
        var_end = malloc(size_end);

        ctx.tiledb_ctx()->handle_error(tiledb_array_get_non_empty_domain_var_from_name(
            ctx.tiledb_ctx()->ptr().get(), array.ptr().get(), dimension.name().c_str(), var_start, var_end, &is_empty));

        auto ned = std::make_pair(std::string((char*)var_start, size_start), std::string((char*)var_end, size_end));
        free(var_start);
        free(var_end);

        return std::make_any<std::optional<std::pair<std::string, std::string>>>(ned);
    }

    void* fixed_ned = malloc(16);
    ctx.tiledb_ctx()->handle_error(tiledb_array_get_non_empty_domain_from_name(
        ctx.tiledb_ctx()->ptr().get(), array.ptr().get(), dimension.name().c_str(), fixed_ned, &is_empty));
    ...
    switch (dimension.type()) {
        case TILEDB_UINT8: {
            if (is_empty) {
                return std::make_any<std::optional<std::pair<uint8_t, uint8_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((uint8_t*)fixed_ned)[0], ((uint8_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<uint8_t, uint8_t>>>(data);
            }
        }
        case TILEDB_UINT16:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<uint16_t, uint16_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((uint16_t*)fixed_ned)[0], ((uint16_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<uint16_t, uint16_t>>>(data);
            }
        case TILEDB_UINT32:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<uint32_t, uint32_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((uint32_t*)fixed_ned)[0], ((uint32_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<uint32_t, uint32_t>>>(data);
            }
        case TILEDB_UINT64:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<uint64_t, uint64_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((uint64_t*)fixed_ned)[0], ((uint64_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<uint64_t, uint64_t>>>(data);
            }
        case TILEDB_INT8:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<int8_t, int8_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((int8_t*)fixed_ned)[0], ((int8_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<int8_t, int8_t>>>(data);
            }
        case TILEDB_INT16:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<int16_t, int16_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((int16_t*)fixed_ned)[0], ((int16_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<int16_t, int16_t>>>(data);
            }
        case TILEDB_INT32:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<int32_t, int32_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((int32_t*)fixed_ned)[0], ((int32_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<int32_t, int32_t>>>(data);
            }
        case TILEDB_DATETIME_YEAR:
        case TILEDB_DATETIME_MONTH:
        case TILEDB_DATETIME_WEEK:
        ...
        case TILEDB_INT64:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<int64_t, int64_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((int64_t*)fixed_ned)[0], ((int64_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<int64_t, int64_t>>>(data);
            }
        case TILEDB_FLOAT32:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<float_t, float_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((float_t*)fixed_ned)[0], ((float_t*)fixed_ned)[1]);
                free(fixed_ned);
                return std::make_any<std::optional<std::pair<float_t, float_t>>>(data);
            }
        case TILEDB_FLOAT64:
            if (is_empty) {
                return std::make_any<std::optional<std::pair<double_t, double_t>>>(std::nullopt);
            } else {
                auto data = std::make_pair(((double_t*)fixed_ned)[0], ((double_t*)fixed_ned)[1]);
                free(fixed_ned);
...
std::any SOMADimension::_core_current_domain_slot(NDRectangle& ndrect) const {
    switch (dimension.type()) {
        case TILEDB_UINT8: {
            ...
            return std::make_any<std::pair<uint8_t, uint8_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_UINT16: {
            std::array<uint16_t, 2> domain = ndrect.range<uint16_t>(dimension.name());
            return std::make_any<std::pair<uint16_t, uint16_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_UINT32: {
            std::array<uint32_t, 2> domain = ndrect.range<uint32_t>(dimension.name());
            return std::make_any<std::pair<uint32_t, uint32_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_UINT64: {
            std::array<uint64_t, 2> domain = ndrect.range<uint64_t>(dimension.name());
            return std::make_any<std::pair<uint64_t, uint64_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_INT8: {
            std::array<int8_t, 2> domain = ndrect.range<int8_t>(dimension.name());
            return std::make_any<std::pair<int8_t, int8_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_INT16: {
            std::array<int16_t, 2> domain = ndrect.range<int16_t>(dimension.name());
            return std::make_any<std::pair<int16_t, int16_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_INT32: {
            std::array<int32_t, 2> domain = ndrect.range<int32_t>(dimension.name());
            return std::make_any<std::pair<int32_t, int32_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_DATETIME_YEAR:
        case TILEDB_DATETIME_MONTH:
        case TILEDB_DATETIME_WEEK:
        ...
        case TILEDB_TIME_AS:
        case TILEDB_INT64: {
            std::array<int64_t, 2> domain = ndrect.range<int64_t>(dimension.name());
            return std::make_any<std::pair<int64_t, int64_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_FLOAT32: {
            std::array<float_t, 2> domain = ndrect.range<float_t>(dimension.name());
            return std::make_any<std::pair<float_t, float_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_FLOAT64: {
            std::array<double_t, 2> domain = ndrect.range<double_t>(dimension.name());
            return std::make_any<std::pair<double_t, double_t>>(std::make_pair(domain[0], domain[1]));
        }
        case TILEDB_STRING_UTF8:
        case TILEDB_STRING_ASCII: {
            std::array<std::string, 2> domain = ndrect.range<std::string>(dimension.name());
            return std::make_any<std::pair<std::string, std::string>>(std::make_pair(domain[0], domain[1]));
        }
        default:
            throw TileDBSOMAError(
                fmt::format("[SOMADimension] Unknown dimension type {}", impl::type_to_str(dimension.type())));
...
std::pair<ArrowArray*, ArrowSchema*> SOMADimension::arrow_domain_slot(
    ...
    switch (domain_type().value()) {
        ...
        case TILEDB_TIME_AS:
        case TILEDB_INT64:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<int64_t>(ctx, array, kind));
            break;
        case TILEDB_UINT64:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<uint64_t>(ctx, array, kind));
            break;
        case TILEDB_INT32:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<int32_t>(ctx, array, kind));
            break;
        case TILEDB_UINT32:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<uint32_t>(ctx, array, kind));
            break;
        case TILEDB_INT16:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<int16_t>(ctx, array, kind));
            break;
        case TILEDB_UINT16:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<uint16_t>(ctx, array, kind));
            break;
        case TILEDB_INT8:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<int8_t>(ctx, array, kind));
            break;
        case TILEDB_UINT8:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<uint8_t>(ctx, array, kind));
            break;
        case TILEDB_FLOAT64:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<double>(ctx, array, kind));
            break;
        case TILEDB_FLOAT32:
            arrow_array = ArrowAdapter::make_arrow_array_child(domain_slot<float>(ctx, array, kind));
            break;
        case TILEDB_STRING_ASCII:
        case TILEDB_STRING_UTF8:
...
void SOMADimension::serialize(nlohmann::json& columns_schema) const {
    ...
    column[TILEDB_SOMA_SCHEMA_COL_TYPE_KEY] = static_cast<uint32_t>(soma_column_datatype_t::SOMA_COLUMN_DIMENSION);
