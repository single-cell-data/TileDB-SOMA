...
std::tuple<std::unique_ptr<std::byte[]>, std::unique_ptr<uint64_t[]>, std::unique_ptr<uint8_t[]>> dictionary_to_values(
    ...
    std::unique_ptr<std::uint8_t[]> validity_buffer = nullptr;
    if (schema->flags & ARROW_FLAG_NULLABLE) {
        validity_buffer = bitmap_to_bytemap(
            static_cast<const uint8_t*>(array->buffers[0]), array->length, array->offset);
    }

    return std::make_tuple<std::unique_ptr<std::byte[]>, std::unique_ptr<uint64_t[]>, std::unique_ptr<uint8_t[]>>(
        std::move(data_buffer), nullptr, std::move(validity_buffer));
}
