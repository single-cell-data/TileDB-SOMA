...
TEST_CASE("ManagedQuery: Copy constructor and assignment test") {
    std::string uri = "mem://unit-test-copy-array";
    auto ctx = std::make_shared<Context>();
    auto [array, d0, a0, _] = create_array(uri, *ctx);
    ...
    auto mq2 = ManagedQuery(mq1);
    REQUIRE(mq2.column_names().size() == 1);
    REQUIRE(mq2.column_names()[0] == "a0");
    
    // Test assignment operator
    auto mq3 = ManagedQuery(array, ctx);
    mq3 = mq1;
    REQUIRE(mq3.column_names().size() == 1);
    REQUIRE(mq3.column_names()[0] == "a0");
}
...
TEST_CASE("ManagedQuery: Query condition test") {
    ...
    // Write some test data
    {
        ...
        write_query.set_layout(TILEDB_UNORDERED)
            ...
            .set_data_buffer("value", value_data);
        write_query.submit();
        write_array.close();
    }
