...
TEST_CASE("Test SOMAQueryCondition on SparseArray", "[SOMAQueryCondition][SOMASparseNDArray]") {
    ...
    // Empty region: invalid - no points.
    {
        ...
        CHECK_THROWS_AS(
            SOMAQueryCondition::create_from_points<int64_t>(*tiledb_ctx, "soma_dim_0", points), std::invalid_argument);
    }

    // Empty region: out-of-bounds points.
    {
        std::vector<int64_t> points{25, 27, 19, 30};
        auto qc = SOMAQueryCondition::create_from_points<int64_t>(*tiledb_ctx, "soma_dim_0", points);
        CHECK(qc.is_initialized());
        check_empty_query_condition(qc, "Range out of bounds: expect no values.");
    }
...
TEST_CASE(
    "Test SOMAQueryCondition on SOMADataFrame with string index column",
    "[SOMAQueryCondition][SOMADataFrame][string-index]") {
    // Create a TileDB sparse array with 1 integer dimensions.
    auto ctx = std::make_shared<SOMAContext>();
    auto tiledb_ctx = ctx->tiledb_ctx();
    ...
    {
        auto qc = SOMAQueryCondition::create_from_range(*tiledb_ctx, "label", std::string("a"), std::string("z"));
        Subarray subarray(*tiledb_ctx, array);
...

    {
        ...
        CHECK_THROWS_AS(
            SOMAQueryCondition::create_from_range(*tiledb_ctx, "label", std::string("fig"), std::string("apple")),
...

    {
        ...
        CHECK_THROWS_AS(SOMAQueryCondition::create_from_points(*tiledb_ctx, "label", points), std::invalid_argument);
    }

    {
        auto qc = SOMAQueryCondition::create_from_range(*tiledb_ctx, "label", std::string("ca"), std::string("fa"));
        Subarray subarray(*tiledb_ctx, array);
...

    {
        ...
        auto qc = SOMAQueryCondition::create_from_points(*tiledb_ctx, "label", points);
        Subarray subarray(*tiledb_ctx, array);
    ...
    {
        auto qc = SOMAQueryCondition::create_from_range(*tiledb_ctx, "label", std::string("g"), std::string("z"));
...
TEST_CASE(
    "Test SOMACoordQueryCondition on SOMADataFrame with string index column",
    "[SOMAQueryCondition][SOMADataFrame][string-index]") {
    // Create a TileDB sparse array with 1 integer dimensions.
    auto ctx = std::make_shared<SOMAContext>();
    auto tiledb_ctx = ctx->tiledb_ctx();
