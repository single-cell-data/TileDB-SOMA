...
TEST_CASE("SOMACoordinateSpace: invalid initialization", "[metadata][spatial]") {
    ...
    CHECK_THROWS_AS(SOMACoordinateSpace(empty_axis_names_, empty_axis_units_), TileDBSOMAError);

    // Check all three constructors throw for repeat names.
    std::vector<SOMAAxis> repeat_axes_{{"x_axis", "meter"}, {"x_axis", "nanometer"}};
    ...
    CHECK_THROWS_AS(SOMACoordinateSpace(repeat_axis_names_, repeat_axis_units_), TileDBSOMAError);
    ...
    CHECK_THROWS_AS(SOMACoordinateSpace(axis_names_len3_, axis_units_len2_), TileDBSOMAError);
    CHECK_THROWS_AS(SOMACoordinateSpace(axis_names_len3_, empty_axis_units_), TileDBSOMAError);
    ...
    CHECK_THROWS_AS(SOMACoordinateSpace(axis_names_reserved_, axis_units_len2_), TileDBSOMAError);
...
TEST_CASE("SOMACoordinateSpace: compare constructors", "[metadata][spatial]") {
    ...
    SOMACoordinateSpace expected1({SOMAAxis{"x", std::nullopt}, SOMAAxis{"y", std::nullopt}});
    ...
    std::vector<std::optional<std::string>> units2{"meters", "meters", "meters"};
    SOMACoordinateSpace expected2({SOMAAxis{"x1", "meters"}, SOMAAxis{"x2", "meters"}, SOMAAxis{"x3", "meters"}});
...
TEST_CASE("SOMACoordinateSpace: from_metadata", "[metadata][spatial]") {
    ...
    std::pair<std::string, SOMACoordinateSpace> test_data = GENERATE(
        ...
        std::make_pair(
            ...
            "\"hour\"}]",
            SOMACoordinateSpace({SOMAAxis{"z", std::nullopt}, SOMAAxis{"t", "hour"}})),
        std::make_pair(
            "[{\"name\":\"z\",\"unit\":\"meter\"},{\"name\":\"t\",\"unit\":"
            "\"hour\"}]",
            SOMACoordinateSpace({SOMAAxis{"z", "meter"}, SOMAAxis{"t", "hour"}})));
...
TEST_CASE("SOMACoordinateSpace: mock metadata round-trip", "[metadata][spatial]") {
    auto coord_space = GENERATE(
        SOMACoordinateSpace(),
        SOMACoordinateSpace({SOMAAxis{"x_axis", "meter"}, SOMAAxis{"y_axis", "meter"}, SOMAAxis{"z_axis", "meter"}}),
