...
based on the git history/tags. It also stores that info in a VERSION
...
To use this script with scikit-build-core (current build system):

    # Generate VERSION file for scikit-build-core
    python _get_version.py > VERSION

The VERSION file should be included in sdist tarballs (via MANIFEST.in).
...
VERSION_FILE = join(pathlib.Path(__file__).parent, "VERSION")
...
def read_version_file() -> str | None:
    ...
    try:
        ...
        if not re.search(_PEP386_VERSION_RE, ver):
            err(f"version: version ({ver}) is invalid, will use it anyway\n")
        return ver
...
def write_version_file(version):
    ...
    Note: In the current build system (scikit-build-core), the VERSION file
    is generated by the build process (via _get_version.py). This function
    is kept for compatibility but may not be called in all scenarios.
    """
...
def get_version():
    """Get the current version from git, or fall back to VERSION file or calver.

    Priority:
    1. Git version (from tags/commits)
    2. VERSION file (for sdist/distributed code)
    3. Calendar version (last resort)
    """
    ...
    if not version:
        ...
        err(f"No {pathlib.Path(VERSION_FILE).name} or Git version found, using calver {version}")
