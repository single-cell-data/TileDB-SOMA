...
def test_import_anndata(adata):
    ...
    # assert sorted(soma.obs_ids()) == sorted(list(orig.obs_names))
    assert sorted([e.decode('utf-8') for e in soma.obs_ids()]) == sorted(list(orig.obs_names))
    ...
    # TODO: the left-hand side is a list of b'...' and the right-hand side is a list of '...'.
    # assert sorted(soma.var_ids()) == sorted(list(orig.var_names))
    assert sorted([e.decode('utf-8') for e in soma.var_ids()]) == sorted(list(orig.var_names))
