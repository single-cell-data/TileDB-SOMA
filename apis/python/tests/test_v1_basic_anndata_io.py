...
def test_import_anndata(adata):
    ...
    with tiledb.Group(output_path) as G:
        assert (
            G.meta[tiledbsc.v1.util.SOMA_OBJECT_TYPE_METADATA_KEY] == "SOMAExperiment"
        )

    # Check obs
    df = exp.obs.to_pandas_all()
    ...
    #        assert A.ndim == 2
    #        assert A.meta[tiledbsc.v1.util.SOMA_OBJECT_TYPE_METADATA_KEY] == "AssayMatrix"
    ...
    #        assert (
    #            A.meta[tiledbsc.v1.util.SOMA_OBJECT_TYPE_METADATA_KEY] == "AnnotationDataFrame"
    #        )
    ...
    #            assert (
    #                A.meta[tiledbsc.v1.util.SOMA_OBJECT_TYPE_METADATA_KEY]
    #                == "AnnotationMatrix"
    #            )
...
#
#    readback = tiledbsc.v1.io.to_anndata(exp)
#
