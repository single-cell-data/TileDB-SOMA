...
def create_and_populate_obs(obs: soma.DataFrame) -> soma.DataFrame:
    ...
    obs._legacy_create(schema=obs_arrow_schema)
...
def create_and_populate_var(var: soma.DataFrame) -> soma.DataFrame:
    ...
    var._legacy_create(schema=var_arrow_schema)
...
def create_and_populate_sparse_nd_array(
    ...
    sparse_nd_array._legacy_create(pa.int64(), [nr, nc])
...
def test_experiment_obs_type_constraint(tmp_path):
    ...
    se = soma.Experiment(uri=tmp_path.as_uri())._legacy_create()

    with pytest.raises(TypeError):
        se["obs"] = soma.Collection(uri=(tmp_path / "A").as_uri())._legacy_create()
    with pytest.raises(TypeError):
        se["obs"] = soma.SparseNDArray(uri=(tmp_path / "B").as_uri())._legacy_create(
            type=pa.float32(), shape=(10,)
        )
    with pytest.raises(TypeError):
        se["obs"] = soma.DenseNDArray(uri=(tmp_path / "C").as_uri())._legacy_create(
            type=pa.float32(), shape=(10,)
        )
    with pytest.raises(TypeError):
        se["obs"] = soma.Measurement(uri=(tmp_path / "D").as_uri())._legacy_create()
    se["obs"] = soma.DataFrame(uri=(tmp_path / "E").as_uri())._legacy_create(
        schema=pa.schema([("A", pa.int32())])
    )
    se["obs"] = soma.DataFrame(uri=(tmp_path / "F").as_uri())._legacy_create(
...
def test_experiment_ms_type_constraint(tmp_path):
    ...
    se["ms"] = soma.Collection(uri=(tmp_path / "A").as_uri())._legacy_create()
    with pytest.raises(TypeError):
        se["ms"] = soma.SparseNDArray(uri=(tmp_path / "B").as_uri())._legacy_create(
            type=pa.float32(), shape=(10,)
        )
    with pytest.raises(TypeError):
        se["ms"] = soma.DenseNDArray(uri=(tmp_path / "C").as_uri())._legacy_create(
            type=pa.float32(), shape=(10,)
        )
    with pytest.raises(TypeError):
        se["ms"] = soma.Measurement(uri=(tmp_path / "D").as_uri())._legacy_create()
    with pytest.raises(TypeError):
        se["ms"] = soma.DataFrame(uri=(tmp_path / "E").as_uri())._legacy_create(
            schema=pa.schema([("A", pa.int32())])
        )
    with pytest.raises(TypeError):
        se["ms"] = soma.DataFrame(uri=(tmp_path / "F").as_uri())._legacy_create(
            schema=pa.schema([("A", pa.int32())]), index_column_names=["A"]
        )
