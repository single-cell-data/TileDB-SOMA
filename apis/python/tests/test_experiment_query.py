...
def test_annotation_matrix_slots_expand(version, obsm_layers, obsp_layers, varm_layers, varp_layers) -> None:
    ...
    def load_joinids(df: soma.DataFrame, axq: AxisQuery) -> pa.IntegerArray:
        tbl = df.read(
            axq.coords,
    ...
    class TestExperimentAxisQuery:
        ...
        @property
        def _obs_df(self) -> soma.DataFrame:
            return self.experiment.obs

        @property
        def _ms(self) -> soma.Measurement:
            return self.experiment.ms[self.measurement_name]

        @property
        def _var_df(self) -> soma.DataFrame:
            return self._ms.var
        ...
        def _get_annotation_layer(self, annotation_name: str, layer_name: str) -> soma.SparseNDArray:
            ...
            """
            try:
                coll: soma.Collection[npt.NDArray] = self._ms[annotation_name]  # type: ignore
            except KeyError:
                raise ValueError(f"Measurement does not contain {annotation_name!r} data.") from None
            if not isinstance(coll, soma.Collection):
        ...
        def to_anndata(
            ...
            *,
            ...
            all_x_arrays: dict[str, soma.SparseNDArray] = {}
            for _xname in all_x_names:
                ...
                if not isinstance(x_array, soma.SparseNDArray):
...
@pytest.mark.parametrize("version", ["1.7.3"] * 750)  #  ["1.7.3", "1.12.3", "1.14.5", "1.15.0", "1.15.7"])
@pytest.mark.parametrize(
    "obsm_layers", [("X_draw_graph_fr", "X_pca", "X_tsne", "X_umap")]
)  #  [(), ("X_pca",), ("X_tsne",), ("X_draw_graph_fr", "X_pca", "X_tsne", "X_umap")])
@pytest.mark.parametrize(
