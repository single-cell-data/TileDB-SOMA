...
def debug() -> None:
    """
    Sets tiledbsoma logging to a DEBUG level. Use ``tiledbsoma.logging.debug()`` in notebooks to see more
...
def log_io(info_message: Optional[str], debug_message: str) -> None:
    """
    Data-ingestion timeframes range widely.  Some folks won't want details in the former; some will
    want details in the latter.  For I/O and for I/O only, it's helpful to print a short message at
    ``INFO`` level, or a different, longer message at/beyond ``DEBUG`` level.
    """
