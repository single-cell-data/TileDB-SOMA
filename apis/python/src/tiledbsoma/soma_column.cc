                ...
                for (const pybind11::handle array_handle : array_chunks) {
                    ...
                    try {
                        if (!strcmp(arrow_schema.format, "l")) {
                            column->set_dim_points<int64_t>(mq, coords.cast<std::vector<int64_t>>());
                        } else if (!strcmp(arrow_schema.format, "i")) {
                            column->set_dim_points<int32_t>(mq, coords.cast<std::vector<int32_t>>());
                        } else if (!strcmp(arrow_schema.format, "s")) {
                            column->set_dim_points<int16_t>(mq, coords.cast<std::vector<int16_t>>());
                        } else if (!strcmp(arrow_schema.format, "c")) {
                            column->set_dim_points<int8_t>(mq, coords.cast<std::vector<int8_t>>());
                        } else if (!strcmp(arrow_schema.format, "L")) {
                            column->set_dim_points<uint64_t>(mq, coords.cast<std::vector<uint64_t>>());
                        } else if (!strcmp(arrow_schema.format, "I")) {
                            column->set_dim_points<uint32_t>(mq, coords.cast<std::vector<uint32_t>>());
                        } else if (!strcmp(arrow_schema.format, "S")) {
                            column->set_dim_points<uint16_t>(mq, coords.cast<std::vector<uint16_t>>());
                        } else if (!strcmp(arrow_schema.format, "C")) {
                            column->set_dim_points<uint8_t>(mq, coords.cast<std::vector<uint8_t>>());
                        } else if (!strcmp(arrow_schema.format, "f")) {
                            column->set_dim_points<float_t>(mq, coords.cast<std::vector<float_t>>());
                        } else if (!strcmp(arrow_schema.format, "g")) {
                            column->set_dim_points<double_t>(mq, coords.cast<std::vector<double_t>>());
                        } else if (
                            !strcmp(arrow_schema.format, "u") || !strcmp(arrow_schema.format, "U") ||
                            !strcmp(arrow_schema.format, "z") || !strcmp(arrow_schema.format, "Z")) {
                            column->set_dim_points<std::string>(mq, coords.cast<std::vector<std::string>>());
                        } else if (
