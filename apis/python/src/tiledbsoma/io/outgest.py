...
from .._dask import DaskConfig, load_daskarray
...
def _extract_X_key(
    ...
    if dask:
        return load_daskarray(
            layer=X,
            chunk_size=dask.chunk_size,
            tdb_concurrency=dask.tdb_concurrency,
            **dask.tdb_configs,
        )
...
def to_anndata(
    ...
    * ``obsp``, ``varp`` arrays as ``scipy.sparse.csr_matrix``
    ...
    anndata_X = (
        ...
        else anndata_X_future if anndata_X_future else None
    )
