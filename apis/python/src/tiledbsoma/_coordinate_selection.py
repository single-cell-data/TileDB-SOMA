...
class CoordinateValueFilters:
    ...
    def create(
        ...
        if len(coords) > len(array._handle_wrapper._handle.dimension_names):
            raise ValueError(
                f"The coords ({len(coords)} elements) must be shorter than the number of index columns ({len(array._handle_wrapper._handle.dimension_names)})",
            )
    ...
    def __attrs_post_init__(self) -> None:
        object.__setattr__(self, "_handle", clib.CoordinateValueFilters(self._array._handle_wrapper._handle))

    def add_coordinate_selection(
        self, column_index: int, coord: options.SparseDFCoord | options.SparseNDCoord | options.DenseCoord
    ) -> None:
        array_handle = self._array._handle_wrapper._handle
