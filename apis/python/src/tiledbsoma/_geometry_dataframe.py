    ...
    def create(
        ...
        *,
        ...
        if soma_domain is None:
            raise TypeError("Cannot set domain=None. Please specify the desired domain for the geometry dataframe.")

        if len(soma_domain) != 2:
            raise ValueError(
                f"Invalid domain={soma_domain}. The domain must have two entries: one for the geometry and one for the soma_joinid."
            )
