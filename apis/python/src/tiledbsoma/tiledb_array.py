...
class TileDBArray(TileDBObject):
    ...
    def _sub_open(self) -> None:
        ...
        if self._open_mode == "r":
            ...
            if (
                self.context.read_timestamp is not None
                or self.context.read_timestamp_start is not None
            ):
                # writers need to be able to read their own writes
                raise ValueError(
                    "SOMATileDBContext read timestamp must not be set while writing"
                )
            timestamp = self.context._write_timestamp  # usually None
        self._open_tiledb_array = self._close_stack.enter_context(
            tiledb.open(
                self._uri, mode=self._open_mode, ctx=self._ctx, timestamp=timestamp
            )
        )
