...
void init_soma_dataframe(py::module &m) {
    ...
    .def_static("create", py::overload_cast<std::string_view, ArraySchema, std::map<std::string, std::string>>(&SOMADataFrame::create))
    .def_static("create", py::overload_cast<std::string_view, ArraySchema, std::shared_ptr<Context>>(&SOMADataFrame::create))
