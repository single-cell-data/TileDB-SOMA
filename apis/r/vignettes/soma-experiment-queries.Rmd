...
We can see our current experiment contains only a single measurement: `"RNA"`.
...
Once created, we can use the `query` object to inspect, select, and extract filtered data from the experiment.
...
Adding filters requires creating a `SOMAAxisQuery` object that allows you to define both coordinates and value filters for an axis.
