...
We can also extract any data component from the experiment. Here we'll read in the `obs` 
data frame from the query using `obs()` which returns an iterator of `arrow::Table`. The iterator is useful 
when the data is too large to load in memory allowing to stream the data in chunks. This 
...
obs
```
As a reminder an `arrow::Table` can be easily cast into a `tibble`

```{r}
as.data.frame(obs)
```

Alternatively, you can use the iterator, which retrieves data in chunks that are smaller than 
the `soma.init_buffer_bytes` context field. You can use the iterator's method `$read_next()` to 
load a chunk in memory. In this example the data is so small that it all fits in one chunk.
...
We can also access the expression data via `X()`. 

Similar to `obs()` and `var()`, `X()` is intended for iteration, but in this case we have 
access to two different iterators, and thus `X()` returns a reader that gives you access to an 
iterator for `arrow::Table` and one for `Matrix::sparse_matrix`.

Let's take a look at the Arrow Table iterator:

```{r}
reader <- query$X(layer_name = "counts")
table_iterator <- reader$tables()
table_iterator$read_next()
```

As in the `obs` example the data is small enough to fit in one chunk. For bigger data
...
The iterator for  `Matrix::sparse_matrix` works in the same way. Keep in mind that the
sparse matrix format is `dgTMatrix`, which aligns with the COO encoding TileDB uses on disk and can be concatenated efficiently over each iteration. 

Importantly, the resulting object is a "view" of the full matrix on-disk, retaining its original shape and indices but containing only data corresponding to the query coordinates or filters (see section below).
...
Adding filters requires creating a `SOMAAxisQuery` object that allows you to define coordinates, 
value filters, or both for an axis.

Here we'll create a query for `obs` that slices the first 40 rows, and then filters that subset 
based on the `nCount_RNA` column.
...
```{r}
as.data.frame(query$obs(column_names = c("obs_id", "nCount_RNA"))$concat())
```
...
Remember that the resulting object is a "view" of the full matrix with the 
original shape and indexes but only with data corresponding to the query coordinates or filters.
