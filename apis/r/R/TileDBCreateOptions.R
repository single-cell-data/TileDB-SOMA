...
DEFAULT_OFFSETS_FILTERS <- function() { c(
    "DOUBLE_DELTA",
    "BIT_WIDTH_REDUCTION",
    "ZSTD"
)}
DEFAULT_VALIDITY_FILTERS <- function() { list() }
DEFAULT_DIM_FILTERS      <- function() { list() }
DEFAULT_ATTR_FILTERS     <- function() { list() }
...
TileDBCreateOptions <- R6::R6Class(
  ...
  public = list(
    ...
    # This allows a user to specify TileDB extents on a per-diimension basis.
    #
    # Example:
    ...
    #
    #' @return int
    dim_tile = function(dim, default=DEFAULT_TILE_EXTENT()) {
      o <- self$.dim(dim)
      if (is.null(o)) {
        return(default)
      }
    ...
    #' @return list of tiledb.Filter
    dim_filters = function(default=DEFAULT_DIM_FILTERS()) {
      self$.build_filters(super$get("dim_filters", default))
    },

    #' @return list of tiledb.Filter
    attr_filters = function(default=DEFAULT_ATTR_FILTERS()) {
      self$.build_filters(super$get("attr_filters", default))
    },
    ...
    #   tdco <- TileDBCreateOptions$new(cfg)
    #
    # -- this pulls out the `dims` -> `soma_dim_0` part.
    #
