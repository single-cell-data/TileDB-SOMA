...
inline std::map<std::string, std::string> config_vector_to_map(
    Rcpp::Nullable<Rcpp::CharacterVector> config) {
    std::map<std::string, std::string> platform_config;

    if (!config.isNull()) {
        Rcpp::CharacterVector confvec(config.get());
        Rcpp::CharacterVector namesvec = confvec.attr(
            "names");  // extract names from named R vector
        size_t n = confvec.length();
        for (size_t i = 0; i < n; i++) {
            platform_config.emplace(std::make_pair(
                std::string(namesvec[i]), std::string(confvec[i])));
            spdl::trace(
                "[config_vector_to_map] adding '{}' = '{}'",
                std::string(namesvec[i]),
                std::string(confvec[i]));
        }
    }
...
struct ContextWrapper {
    // ContextWrapper(std::shared_ptr<tiledb::Context> ctx_ptr_) :
    // ctxptr(std::move(ctx_ptr_)) {}
    ContextWrapper(std::shared_ptr<tiledb::Context> ctx_ptr_)
        : ctxptr(ctx_ptr_) {
    }
