...
Rcpp::XPtr<ArrowArray> array_setup_struct(
    Rcpp::XPtr<ArrowArray> arrxp, int64_t n_children);
...
//' @param timestamp_end Optional POSIXct (i.e. Datetime) type for end of
//interval for which ' data is considered. ' @param sr An external pointer to a
//TileDB SOMAArray object
//'
...
Rcpp::List sr_setup(
    ...
    std::map<std::string, std::string> platform_config = config_vector_to_map(
        Rcpp::wrap(config));
    tiledb::Config cfg(platform_config);
    spdl::debug("[sr_setup] creating ctx object with supplied config");
    std::shared_ptr<tiledb::Context> ctxptr = std::make_shared<tiledb::Context>(
        cfg);
    ...
    std::uint64_t ts_start = 0;  // beginning of time aka the epoch (force
                                 // double signature)
    std::uint64_t
        ts_end = std::numeric_limits<uint64_t>::max();  // max if unset
    if (!timestamp_end.isNull()) {
        ts_end = Rcpp::as<Rcpp::Datetime>(timestamp_end)
    ...
    return Rcpp::List::create(
        Rcpp::Named("sr") = xptr, Rcpp::Named("ctx") = ctx_wrap_xptr);
}
...
Rcpp::List create_empty_arrow_table() {
    ...
    arrayxp->length = 0;
    Rcpp::List as = Rcpp::List::create(
        Rcpp::Named("array_data") = arrayxp, Rcpp::Named("schema") = schemaxp);
    return as;
}
...
Rcpp::List sr_next(Rcpp::XPtr<tdbs::SOMAArray> sr) {
    ...
    for (size_t i = 0; i < ncol; i++) {
        // this allocates, and properly wraps as external pointers controlling
        // lifetime
