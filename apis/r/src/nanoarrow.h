...
static inline void ArrowDecimalGetBytes(
    struct ArrowDecimal* decimal, uint8_t* out) {
...
#define ArrowErrorMessage \
    NANOARROW_SYMBOL(NANOARROW_NAMESPACE, ArrowErrorMessage)
...
int64_t ArrowSchemaToString(
    struct ArrowSchema* schema, char* out, int64_t n, char recursive);
...
ArrowErrorCode ArrowSchemaDeepCopy(
    struct ArrowSchema* schema, struct ArrowSchema* schema_out);
...
static inline ArrowErrorCode ArrowArrayAppendDecimal(
    struct ArrowArray* array, struct ArrowDecimal* value);
...
/// buffer content was obtained from an untrusted or corruptable source.
...
ArrowErrorCode ArrowArrayViewValidateFull(
...
static inline void ArrowArrayViewGetDecimalUnsafe(
    struct ArrowArrayView* array_view, int64_t i, struct ArrowDecimal* out);
...
ArrowErrorCode ArrowBasicArrayStreamValidate(
    struct ArrowArrayStream* array_stream, struct ArrowError* error);
...
static inline void _ArrowBitmapPackInt8(const int8_t* values, uint8_t* out) {
    *out =
        (values[0] | values[1] << 1 | values[2] << 2 | values[3] << 3 |
...
static inline int8_t _ArrowParseUnionTypeIds(
...
static inline ArrowErrorCode ArrowArrayAppendInt(
    ...
    switch (private_data->storage_type) {
        ...
        case NANOARROW_TYPE_DOUBLE:
            NANOARROW_RETURN_NOT_OK(
                ArrowBufferAppendDouble(data_buffer, value));
            break;
        case NANOARROW_TYPE_FLOAT:
