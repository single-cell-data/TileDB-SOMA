pbmc3k_sce <- function() {
  ...
  load(
    file = system.file("data", "pbmc3k.rda", package = "pbmc3k", mustWork = TRUE),
    envir = e
  )
  load(
    file = system.file("data", "pbmc3k.sce.logcounts.rda", package = "pbmc3k", mustWork = TRUE),
    envir = e
  )
  ...
  clusters <- system.file("extdata", "pbmc3k-sce", "clusters.txt", package = "tiledbsoma")
  if (nzchar(clusters)) {
    SingleCellExperiment::colLabels(sce) <- as.factor(readLines(clusters))
  }
  decode <- function(uri) {
    stopifnot(nzchar(uri), file.exists(uri))
    x <- as.matrix(read.csv(uri, header = TRUE, row.names = 1L, colClasses = "character"))
    return(matrix(
      as.numeric(x),
      nrow = nrow(x),
      ncol = ncol(x),
  ...
  umap <- system.file("extdata", "pbmc3k-sce", "umap.csv", package = "tiledbsoma")
  if (nzchar(umap)) {
