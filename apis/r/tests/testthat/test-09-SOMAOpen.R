test_that("SOMAOpen", {
  ...
  expect_s3_class(
    SOMAOpen(file.path(uri, "pbmc3k_processed")),
    "SOMAExperiment"
  )

  expect_s3_class(
    SOMAOpen(file.path(uri, "pbmc3k_processed", "ms")),
    "SOMACollection"
  )
  expect_s3_class(
    SOMAOpen(file.path(uri, "pbmc3k_processed", "obs")),
    "SOMADataFrame"
  )

  expect_s3_class(
    SOMAOpen(file.path(uri, "pbmc3k_processed", "ms", "raw")),
    "SOMAMeasurement"
  )
  expect_s3_class(
    SOMAOpen(file.path(uri, "pbmc3k_processed", "ms", "RNA")),
    "SOMAMeasurement"
  )
