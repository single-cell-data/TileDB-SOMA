test_that("SOMADataFrame round-trip with factor and ordered", {
  ...
  et <- structure(
    list(
      ...
      survived = c(
        0L, 0L, 1L, 1L, 1L, 0L, 1L, 1L, 0L, 1L, 1L, 1L, 0L, 1L, 1L, 1L, 1L,
      ...
      parch = c(
        2L, 1L, 0L, 0L, 0L, 0L, 2L, 0L, 0L, 2L, 0L, 2L, 2L, 2L, 0L, 0L, 0L, 1L,
