# Convert a Seurat Sub-Object to a SOMA Object, returned opened for write

Various helpers to write Seurat sub-objects to SOMA objects..
...
- soma_parent:
  ...
  The parent [collection](SOMACollection.md); for the `DimReduc` and
  `Graph` methods, this **must** be a [measurement](SOMAMeasurement.md)
  for the assay `x` was generated from.
...
- fidx:
  ...
  An integer vector describing the location of features in
  `SeuratObject::Loadings(x)` with relation to `soma_parent` (eg.
  `match(rownames(Loadings(x)), rownames(assay))`).

- nfeatures:

  The number of features present in `soma_parent` (eg. `nrow(assay)`).

## Value

`Assay` and `Assay5` methods: a [`SOMAMeasurement`](SOMAMeasurement.md)
with the data from `x`, returned opened for write.

`DimReduc` and `Graph` methods: invisibly returns `soma_parent`, opened
for write, with the values of `x` added to it.
...
#> <SOMAMeasurement>
#>   uri: /tmp/Rtmpf7VWBj/seurat-sub2b79381a2493/RNA
#>   arrays: var 
#>   groups: X 
...
#> <SOMAMeasurement>
#>   uri: /tmp/Rtmpf7VWBj/seurat-sub2b79381a2493/RNA5
#>   arrays: var 
#>   groups: X 
...
#> <SOMACollection>
#>   uri: /tmp/Rtmpf7VWBj/seurat-sub2b79381a2493/seurat_commands
#>   arrays: NormalizeData.RNA 
logs$get("NormalizeData.RNA")
#> <SOMADataFrame>
#>   uri: /tmp/Rtmpf7VWBj/seurat-sub2b79381a2493/seurat_commands/NormalizeData.RNA
