...
SCGroup::SCGroup(std::string& group_uri, std::optional<SCConfig> config)
        :  ctx_(tiledb::Context()), uri_(group_uri) // TODO use config here
    {
    ...
    check_paths_exist(
        vector<string>({X_path, var_path, obs_path}),
        config
    );
...
SCArray::SCArray(shared_ptr<tiledb::Array> array) : array_(std::move(array)) {}
